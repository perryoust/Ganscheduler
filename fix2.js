...
// Let's find exactly this block to know where the good code ends.
const goodEndText = 'reader.readAsArrayBuffer(file);\n};';
const goodEndIndex = content.indexOf(goodEndText);

if (goodEndIndex !== -1) {
  const cleanContent = content.substring(0, goodEndIndex + goodEndText.length);
  ...
  console.log('Successfully repaired and appended to invoices.js');
} else {
  console.log('Could not find the end marker in invoices.js');
}
