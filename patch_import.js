...
// Hijack importInvoices to support both traditional input and File System API
let newImportInvoices = \window.importInvoices = async function(input) {
...
code = code.replace(/window\.importInvoices = async function\(input\) \{[\s\S]*?if \(!file\) return;[\s\S]*?if \(typeof window\.XLSX === "undefined"\) \{[\s\S]*?_spAlertDialog\("שגיאה: ספריית XLSX לא נטענה\. אנא רענן את הדף\."\);[\s\S]*?return;[\s\S]*?\}/, newImportInvoices);
