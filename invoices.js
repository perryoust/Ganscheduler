    ...
    tbody.innerHTML = renderList.map(inv=>{
      ...
      return `<tr class="inv-row-clickable" style="${_isDup?'background:#fce4ec;border-right:3px solid #c62828;':''}" onclick="openNewInvoice('${inv.id || inv.serialNum}')">
        ...
        <td style="font-size:.75rem;line-height:2;padding:8px">
          ...
          ${(!hasOrder && !hasTx && !hasTax) ? `<div style="display:flex;align-items:center;gap:5px;flex-wrap:wrap">${((inv.file_order && inv.file_order.path) || (inv.file_tx && inv.file_tx.path) || (inv.file_tax && inv.file_tax.path)) ? `<span style="display:inline-flex;align-items:center;gap:3px;background:#e8f5e9;border:1px solid #a5d6a7;border-radius:4px;padding:2px 7px;font-size:.7rem;color:#2e7d32;cursor:pointer;font-weight:600" onclick="event.stopPropagation();invOpenFile('${inv.id || inv.serialNum}','${inv.file_tax&&inv.file_tax.path?'tax':inv.file_tx&&inv.file_tx.path?'tx':'order'}')" title="פתח קובץ">📎 קובץ מצורף ↗</span>` : `<span style="display:inline-flex;align-items:center;gap:2px;background:#fff8e1;border:1px solid #ffe082;border-radius:4px;padding:1px 6px;font-size:.67rem;color:#e65100;cursor:pointer" onclick="event.stopPropagation();openNewInvoice('${inv.id || inv.serialNum}')" title="עדכן קישור לקובץ">📎 עדכן קישור</span>`}</div>` : ''}
        </td>
