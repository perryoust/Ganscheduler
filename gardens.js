...
function saveHoliday(){
  ...
  if(!name||!from||!to){_spAlertDialog('יש למלא שם ותאריכים');return;}
...
function saveClusterModal(){
  ...
  if(!name){_spAlertDialog('יש להזין שם אשכול');return;}
...
function saveClusterSchedule(){
  ...
  save();
  if(saved>0){
    _spAlertDialog(`✅ שובצו ${saved} פעילויות לתאריך ${fD(date)}`);
    CM('clsm');refresh();
  }
}
...
function genExport(){
  ...
  const getCoordStr = (gid) => {
    ...
    const mgr = Object.values(window.managers || {}).find(m => {
        if (!m || !m.gardenIds) return false;
        let arr = m.gardenIds;
...
function printExport(){
  const t=document.getElementById('ex-prev').textContent;
  if(!t||t.startsWith('לחץ')){_spAlertDialog('יש ליצור תצוגה מקדימה תחילה');return;}
