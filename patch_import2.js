...
const regex = /window\.importInvoices = async function\(input\) \{[\s\S]*?if \(!file\) return;\s+if \(typeof window\.XLSX === "undefined"\) \{\s+_spAlertDialog\("שגיאה: ספריית XLSX לא נטענה\. אנא רענן את הדף\."\);\s+return;\s+\}/m;
