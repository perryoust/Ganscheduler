...
const startStr = "if(isSupplierExport){";
const endStr = "} else {";
...
const replacement = `if(isSupplierExport){
        ...
        const globalSumHead = isPlacement ? ws.addRow([globalSummaryTitle, '', '', '', '', '', '', '']) : ws.addRow([globalSummaryTitle, '', '', '', '', '', '', '', '']);
...
txt = txt.substring(0, startIdx) + replacement + txt.substring(endIdx + 8);
