...
### 3.5 שיוך קופה קטנה בסורק SharePoint (`scanner_worker.js`)
- **שיוך קבוצתי (Batch Linking)**: קובץ קופה קטנה משויך לכל שורות הקופה הקטנה של אותה רכזת באותו חודש תקציבי (למשל כל 5 השורות של דנית שאול בספטמבר מקבלות את אותו קובץ).
- **בידוד מוחלט לפי שם הרכזת**: אין לחבר קובץ קופה קטנה לשורות אם שם הרכזת (`inv.supName`) אינו מופיע במפורש בשם הקובץ/נתיב. אין לתת ציון ספק חיובי ללא התאמת שם.
- **קדימות חודש תקציבי על פני תאריך קבלה**: קביעת החודש לשורת קופה קטנה נעשית לפי חודש התקציב/הפעילות (`actMonth`, `orderMonth`, או טקסט "תקציב חודש X" ב-`orderDesc`), כדי שקבלות שנקנו בסוף החודש הקודם עבור תקציב החודש העוקב (למשל קבלה מ-26/08 לתקציב ספטמבר) ישויכו לקובץ הנכון.
- **חיבור דו-ערוצי (Order + Tax)**: שורת קופה קטנה מקבלת את הקישור הן לסלוט ההזמנה (`file_order`) והן לסלוט החשבונית/קבלה (`file_tax`).
...
- **יומן משימות למנהל (`renderWorkerTasksAdmin`)**:
  כולל כפתור תאריך אינטראקטיבי המציג "היום" כברירת מחדל או את התאריך הנבחר כשעוברים יום. לחיצה מפעילה את `window.wtOpenDatePicker()` שפותחת את לוח השנה באמצעות `showPicker()`. שדה ה-Date מוסתר ברקע עם `pointer-events: none` כדי למנוע בליעת קליקים. בעת מעבר לתאריך אחר, מופיע כפתור מהיר `↩️ חזור להיום`.

---


## 10. הפקת PDF בעברית, RTL ו-Bidi
### 10.1 בעיות ידועות (pdfMake ו-html2canvas)
**בעיה**: מנוע ה-Bidi הפנימי של ספריות כמו `pdfMake` ו-`html2canvas` אינו מיישם בצורה מושלמת את תקן UAX #9. תופעות הלוואי כוללות:
1. בליעת רווחים (Space swallowing) בין מילים בעברית או בין מספרים למילים.
2. תווים ניטרליים (כמו `-`, `|`, `()`) שמקבלים כיווניות שגויה או מתהפכים.
3. סוגריים שלא מקבלים Mirroring נכון.

### 10.2 פתרון חובה: rtlFix
**כלל קריטי**: אין להשתמש בפונקציות שמבצעות היפוך אותיות ידני (`rev()`) – הן שוברות את עימוד שבירת השורות (Word wrap). כמו כן, אין להשתמש ב-`&nbsp;` כדי למנוע בליעת רווחים, שכן רווח קשיח מוגדר כתו ניטרלי ושובר את הכיווניות של מקפים וסוגריים סמוכים.

**הפתרון המיושם והיחיד שיש להשתמש בו** ליצירת מסמכי PDF (בשימוש עם `html2pdf.js` ו-`html2canvas`) הוא פונקציית `rtlFix` (הזהה לזו שב-`purchasing_orders.js` ו-`purchasing_budget.js`):
- הפונקציה עוטפת מחרוזות LTR טהורות (אנגלית/מספרים) ב-`<span style="direction:ltr">`.
- מחליפה רווחים רגילים באלמנטי `inline-block` פיזיים ש-`html2canvas` אינו יכול לבלוע: `<span style="display:inline-block; width:0.25em;"> </span>`.
